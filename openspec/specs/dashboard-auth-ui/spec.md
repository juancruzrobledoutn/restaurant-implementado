# dashboard-auth-ui Delta Spec

> Change: **C-14 dashboard-shell**. Establishes the authentication UI surface on the Dashboard.
> The previously archived change left requirement stubs in `openspec/specs/dashboard-auth-ui/spec.md`.
> This delta MODIFIES those requirements so the change re-asserts and owns them for this implementation cycle.
...
After successful login, the authStore SHALL start a `setInterval` that calls `POST /api/auth/refresh` every 14 minutes (840,000 ms) with a random jitter of +/- 2 minutes. The refresh endpoint uses the HttpOnly cookie automatically. On success, the new access token SHALL replace the old one in memory. On failure, the store SHALL trigger logout.
...
- **WHEN** multiple requests return HTTP 401 simultaneously
- **THEN** fetchAPI SHALL perform exactly one `POST /api/auth/refresh` call, and every pending request SHALL retry with the resulting access token
