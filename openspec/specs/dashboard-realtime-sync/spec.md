...
Implement real-time synchronization for the Dashboard menu stores by establishing a WebSocket connection to the admin gateway, routing ENTITY_* and CASCADE_DELETE events to Zustand stores, and providing a hook-based interface for components to subscribe to store changes with optimistic update reconciliation.
## Requirements
