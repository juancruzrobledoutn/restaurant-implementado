# dashboard-promotions-page Delta Spec

> Change: **C-27 dashboard-promotions**. Introduces the Promotions CRUD page in the Dashboard, plus the `promotionStore`, two new reusable UI components (`DateRangePicker`, `MultiSelect`), promotion validators, promotion formatters (validity range, status), cascade preview, and the `canManagePromotions` / `canDeletePromotion` permission derivations.
